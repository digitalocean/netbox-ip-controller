...
// DeepCopyInto is normally an autogenerated deepcopy function,
// copying the receiver, writing into out. spec must be non-nil.
// It is added explicitly here b/c https://github.com/kubernetes/gengo
// (responsible for code generation) does not know how to copy netip.Addr.
// That's b/c netip.Addr is internally represented as a `struct` Kind,
// and `gengo` expects that structs implement DeepCopyInto method for copying.
// But since we cannot add a method on netip.Addr, we have to handle it in the
// struct that _contains_ netip.Addr as a field.
